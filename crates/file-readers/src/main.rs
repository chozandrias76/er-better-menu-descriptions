...
pub fn main() {
    ...
    match file {
        Ok(mut file) => {
            ...
            match nav.find_by_key_value_adv(
                "name",
                Some("Wild Strikes"),
                false,
                true,
                ...
                MatchResult::Single(obj) | MatchResult::SingleExact(obj) => {
                    ...
                    match events_nav
                        .find_by_key_value_adv(second_key, None, true, false)
                    {
                        MatchResult::Keys(_keys) => {
                            println!("Multiple matches")
                        }
                        MatchResult::All(all) => {
                            println!("All matches");
                            let file_to_write = path_to_write.clone().join(
                                format!("wild-strikes.{second_key}.json"),
                            );
                            println!(
                                "Writing to file: {}",
                                file_to_write.to_string_lossy()
                            );
                            let mut file =
                                File::create(&file_to_write).unwrap();
                            file.write_all(
                        ...
                        MatchResult::None => {
                            println!(
                                "No match for {second_key} on name 'Wild Strikes'"
                            );
                            // Serialize the current object to a file
                            let file_to_write =
                                path_to_write.clone().join("wild-strikes.json");
                        ...
                        MatchResult::Single(single)
                        | MatchResult::SingleExact(single) => {
                            ...
                            let activeframes_nav = Navigator::new(
                                &activeframes.unwrap().to_string(),
                            );
                            ...
                            match result {
                                ...
                                MatchResult::None => {
                                    println!("No match for activeframes")
                                }
                            }
                        }
                    }
