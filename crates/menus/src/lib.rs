...
use last_weapon::retour::build_hook_sequence;
...
pub unsafe extern "C" fn DllMain(hmodule: HINSTANCE, reason: u32, _: *mut std::ffi::c_void) {
    if reason == windows::Win32::System::SystemServices::DLL_PROCESS_ATTACH {
        ...
        std::thread::spawn(move || {
            ...
            let (data_addr, jump_instruction_to_buffer) = match jump_instructions() {
                Some(value) => value,
            ...
            let get_weapon_hook_ptr_addrss = unsafe { base_address.offset(0x7c045d) };
            ...
            unsafe {
                tracing::info!("Weapon hook pointer address: {:?}", get_weapon_hook_ptr);
            ...
            let arc_wrapped_ptr = Arc::new(Mutex::new(PtrWrapper(get_weapon_hook_ptr_addrss as _)));
            let atomic_usize_data_addr = AtomicUsize::new(data_addr as usize);
            let arc_wrapped_ptr_clone = arc_wrapped_ptr.clone();
            let gui = HookGui::new(message_replacements, atomic_usize_data_addr, move || {
                unsafe {
                    let _ = unhook(&arc_wrapped_ptr_clone, original_instructions);
                };
            });
...
fn jump_instructions() -> Option<(*const u8, [u8; 5])> {
    ...
    let instructions = build_hook_sequence(None);
    ...
    for attempt in 0..40 {
        ...
        if !exec_mem.is_null() {
            tracing::info!("Successfully allocated memory at 0x{:X}", exec_mem as usize);
            break;
        }
    }

    if exec_mem.is_null() {
        tracing::error!("Failed to allocate executable memory after multiple attempts");
        return None;
    }

    let sequence = build_hook_sequence(Some(exec_mem as usize));
    let (_data_addr_offset, instructions) = match sequence {
    ...
    Some((0x13FE00011 as *const u8, jump_instruction))
}

type MessageReplacements = Mutex<Vec<(FmgCategories, u32, Arc<Mutex<Vec<u16>>>)>>;
...
fn create_replacement_message(
    ...
    message_replacements
        .lock()
        .unwrap()
        .push((category, id, replacement_message_ptr));
    message_replacements
}
...
mod test {
    ...
    fn test_create_replacement_message() {
        ...
        let result = crate::create_replacement_message(message_replacements, category, id);
        assert!(
