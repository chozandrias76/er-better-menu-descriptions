...
-> Option<(
    *const u8,
    ...
    let instructions = record_rax_trampoline_data_with_data_ptr(None, Some(return_addr));
    let (
        _data_addr_offset,
        hook_code,
    ) = match instructions {
    ...
    for attempt in 0..40 {
        let offset =
            if attempt == 0 {
                ...
                attempt
                    * 0x100000 // Then try at 1MB intervals
            };
    ...
    let sequence =
        record_rax_trampoline_data_with_data_ptr(Some(exec_mem as usize), Some(return_addr));
    let (
        data_addr_offset,
        instructions,
    ) = match sequence {
...
fn create_replacement_message(
    ...
    let buffer: Vec<u16> =
        replacement_text
            .encode_utf16()
            .collect();
    let buffer_box = buffer
        .into_boxed_slice();
    let replacement_message_ptr =
    ...
    fn test_jump_instructions()
     {
        ...
        assert_eq!(
            buffer[0], 0xE9,
            "First byte should be JMP instruction"
        );
    }
